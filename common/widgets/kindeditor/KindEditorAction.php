...
//有上传文件时
    ...
    public function UploadJosnAction2() {
        $ext_arr = array(
            'image' => array('gif', 'jpg', 'jpeg', 'png', 'bmp'),
            'flash' => array('swf', 'flv'),
            'media' => array('swf', 'flv', 'mp3', 'wav', 'wma', 'wmv', 'mid', 'avi', 'mpg', 'asf', 'rm', 'rmvb'),
        ...
        if (!empty($_FILES)) {
            ...
            //检查是否已上传
            if (@is_uploaded_file($_files['tmp_name']) === false) {
                $this->alert("上传失败。");
            }
            //检查文件大小
            if ($_files['size'] > $this->max_size) {
                $this->alert("上传文件大小超过限制。");
            }

            //获得文件扩展名
            $file_ext = pathinfo($_files['name'])['extension'];
            ...
            //上传到媒体中心目录
            $file_name = md5(microtime() . $username . rand(0, 1)) . '.' . $file_ext;
            $folder = '/' . $shop_id . '/goods/' . $goods_id . '/content';
            $son_dir = '/users/' . $username{0} . '/' . $username{1} . '/' . $username{2} . '/' . $username{3} . '/' . $username . $folder;
            $_all_dir_path = $_cloud_file_root . $son_dir;
            $_ret_path = $folder . '/' . $file_name;

            //创建目录失败
            if (!file_exists($_all_dir_path) && !mkdir($_all_dir_path, 0777, true)) {
                $this->alert("创建目录失败。");
                return;
            } else if (!is_writeable($_all_dir_path)) {
                $this->alert("上传目录没有写权限。");
                return;
            }
            //移动文件
            if (!(move_uploaded_file($_files["tmp_name"], $_all_dir_path . '/' . $file_name) && file_exists($_all_dir_path . '/' . $file_name))) { //移动失败
                $this->alert("文件上传失败。");
            }

            $file_url = rtrim(yiiParams('shop_index_url'), '/') . yiiParams('cloud_box')['thumbnail'] . '?' . http_build_query(['username' => $username, 'path' => $_ret_path, 'width' => '600', 'options' => 'fit', 'watermark' => '水印.png']);
