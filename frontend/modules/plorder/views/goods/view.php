...
<div class="body-All">
    <header>
        <div class="left"><a href="/plorder/home/index"><img src="/images/home.png"></a></div>
        <div class="right">
    ...
    <div class="box">
        ...
        <div class="integral">
            兑换积分：<span><?php echo $goods['plorder_pionts'] ?></span>分
        </div>
        <div class="prompt">
    ...
    <div class="exchange">
        ...
        <div class="button"><a href="javaScript:void(0)" class="btn plorder-now">立即兑换</a></div>
        <div class="button add"><a href="javaScript:void(0)" class="btn add-to-cart">加入购物车</a></div>
    </div>
</div>
<script>
    $(".plorder-now").click(function(){
        var count = parseInt($("input[name=number]").val());
        if(count <= 0){
            return
        }
        var param = {count: count, gid: <?php echo $goods['gid'] ?>};
        $._ajax('/plorder/cart/ajax-add-goods', param, 'POST', 'JSON', function(json){
            if(json.code > 0){
                window.location.href = '/plorder/cart/goods-list';
            }else{
                alert('添加失败');
            }
        });
