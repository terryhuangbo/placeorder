    ...
    TreeView.prototype = {
        ...
        move: function (dir) {
            ...
            if ($nodeFrom.hasClass('kv-empty')) {
                BUI.Message.Alert(msg.nodeNewMove);
                return;
            }
            switch (dir) {
                case 'u':
                ...
                case 'd':
                    ...
                    if ($nodeTo.length === 0) {
                        BUI.Message.Alert(msg.nodeBottom);
                        return;
                    }
                    fnMove = function () {
                ...
                case 'l':
                    ...
                    if ($nodeTo.length === 0) {
                        BUI.Message.Alert(msg.nodeLeft);
                        return;
                    }
                ...
                case 'r':
                    $nodeTo = $nodeFrom.prev();
                    if ($nodeTo.length === 0) {
                        BUI.Message.Alert(msg.nodeRight);
                        return;
                    }
                    fnMove = function () {
                        if ($nodeTo.find('li').length > 0) {
        ...
        create: function () {
            ...
            if ($node.hasClass('kv-disabled')) {
                BUI.Message.Alert(msg.nodeDisabled);
                return;
            }
            if ($nodeText.length === 0 || $node.hasClass('kv-empty')) {
                BUI.Message.Alert(msg.invalidCreateNode);
                return;
            }
