...
<!--                <h2>权限中心</h2>-->
                ...
                <div class="tool">
                    <a class="btn btn-primary btn-sm "
                       href="<?= Yii::$app->urlManager->createUrl('team/privilege/add-group') ?>">添加分组</a>
                </div>
                <table class="table table-hover">
                    ...
                    <?php if (count($privilegesgroups) > 0): ?>
                        <?php foreach ($privilegesgroups as $k => $v): ?>
                            <tr>
                                <td><?= $k ?></td>
                                <td><?= $v->groupname ?></td>
                                ...
                                <td><?= $v->groupdesc ?></td>
                                <td>
                                    <a class="btn btn-sm btn-success" href="<?= Yii::$app->urlManager->createUrl(['team/privilege/update-group', 'id' => $v->id]) ?>">编辑</a>
                                    <a class="btn btn-sm btn-danger msgshow" href="<?= Yii::$app->urlManager->createUrl(['team/privilege/del-group', 'id' => $v->id]) ?>">删除</a>
                                </td>
                            </tr>
                        <?php endforeach ?>
                    <?php else: ?>
