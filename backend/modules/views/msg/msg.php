            ...
            <div class="main">
                ...
                <div class="tool">
                    <a class="btn btn-primary btn-sm"
                       href="<?= Yii::$app->urlManager->createUrl('admin/msg/sendmsg') ?>">发送消息</a>
                </div>
                <table class="table table-hover">
                    ...
                    <?php if (count($msgs) > 0): ?>
                        <?php foreach ($msgs as $v): ?>
                            <tr>
                                <td><?= $v->id ?></td>
                                <td><?php if ($v->reply): ?><?= $v->name->nickname ?>回复了：<b style="color:#D9534F"><?= $v->title ?></b>  <?php else: ?> <?= $v->title ?><?php endif ?>
                                </td>
                                <td><?= date('Y-m-d H:i:s', $v->send_time) ?></td>
                                <td><?= $v->name->nickname ?></td>
                                <td>
                                    <?php if ($v->status): ?>
                                        <a class="btn btn-sm btn-default msgshow" href="javascript:void(0)"
                                           from="<?= $v->name->nickname ?>"
                                           to="<?= Yii::$app->user->identity->nickname ?>" title="<?= $v->title ?>"
                                           content="<?= strip_tags($v->content) ?>"
                                           send_time="<?= date('Y-m-d H:i:s', $v->send_time) ?>">已读</a>
                                    <?php else: ?>
